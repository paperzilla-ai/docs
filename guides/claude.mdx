...
| `feed.get` | Feed items with filters such as `must_read`, `since`, `limit`, and `offset` |
| `feed.atom_url` | The Atom feed URL for one project |
| `paper.get` | Paper metadata and status |
| `paper.markdown` | Paper markdown, or a queued/unavailable status |
