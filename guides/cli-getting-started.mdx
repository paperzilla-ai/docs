...
Homebrew 6.0 and later may require explicit trust for an existing Paperzilla installation. If `brew update` reports that `paperzilla-ai/tap/pz` is from an untrusted tap, run:
...
Trust only the formula. Do not disable Homebrew's tap trust globally. See the [CLI guide](/guides/cli) for full installation and update instructions.
