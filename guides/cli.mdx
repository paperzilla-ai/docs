...
The Paperzilla CLI (`pz`) lets you browse your curated research feed, inspect individual papers, manage projects, and stay on top of new papers from the terminal. It works for both humans and AI agents — use `--json` to get structured output.
...
Show details for a specific paper or feed item.

`id` may be:

- a full paper UUID
- a paper `short_id`
- a feed item / `project_paper` UUID returned by `pz feed --json`
- a feed item / `project_paper` `short_id` returned by `pz feed --json`
...
| `--markdown` | | Print raw paper markdown to stdout |

#### Examples

```bash
# Inspect a paper by UUID or short ID
pz paper <paper-id>

# Fetch a paper using an ID returned by the feed JSON
pz feed <project-id> --json | jq -r '.items[0].id' | xargs pz paper

# Print raw markdown
pz paper <paper-id> --markdown
```

If markdown generation is still pending, the CLI prints a friendly one-line message and asks you to try again in a minute or so.
...
The Paperzilla CLI is also available as an [OpenClaw skill on ClawHub](https://clawhub.ai/pors/paperzilla). This lets AI agents like Claude Code, Cursor, and Windsurf use `pz` commands directly.
