...
Codex works with Paperzilla in two ways. Add Paperzilla MCP to `~/.codex/config.toml` with a static `Authorization` header when you want native tool calling. Then confirm the server in `/mcp`. Use `pz` directly when Codex already works in a shell or repo. A small Codex skill is helpful for either path because it teaches Codex when to prefer MCP, when to use the CLI, and which Paperzilla tool sequence to follow. A plugin is only needed when you want an installable package. See [Use Paperzilla with Codex](/guides/codex).
