...
description: "Connect your MCP client to Paperzilla and access projects, feeds, paper detail, and markdown in natural language."
keywords: ["MCP", "Model Context Protocol", "AI agent", "Claude", "Cursor"]
---
...
- Open one paper by paper ID, paper short ID, feed item ID, or feed item short ID
- Get markdown for one paper, or queue markdown generation if it is not ready yet
...
- "Open paper `<paper-id>`."
- "Get the markdown for paper `<paper-id>`."
...
| `paper_get` | Get one paper by paper ID or feed item ID | `pz paper <id>` |
| `paper_markdown` | Get paper markdown or queue markdown generation | `pz paper <id> --markdown` |
| `feed_title_filter` (optional prompt) | Ask the model to call `feed_get` and apply title keyword filtering client-side | No direct CLI equivalent |
